...
export const GameView = {
    ...
    <div v-else class="pt-16 px-4 pb-24">
                    ...
                    <div class="flex justify-between mb-3">
                        <div>
                            <div class="text-white font-bold flex gap-2 items-center">
                                {{ p.name }} 
                                <button v-if="!p.uid && !amIIn" @click="bind(p)" class="text-[10px] bg-slate-600 px-2 rounded hover:bg-emerald-600">認領</button>
                                <button v-if="!p.uid && amIIn" @click="invite(p)" class="text-[10px] bg-slate-700 text-gray-400 px-2 rounded border border-slate-600 hover:text-white"><i class="fas fa-share-alt mr-1"></i>邀人</button>
                                <span v-if="p.uid" class="text-blue-400 text-[10px]">●</span>
                            </div>
        ...
        <div v-if="showSettlement" class="fixed inset-0 z-50 flex items-end justify-center bg-black/80 p-4" @click.self="showSettlement=false"><div class="bg-slate-800 w-full max-w-sm rounded-2xl p-6 mb-20 flex flex-col max-h-[80vh]">
            ...
            <div v-if="gap!==0" class="text-rose-400 text-center text-xs mb-2">誤差: {{ formatNumber(gap) }}</div>
            <div class="grid gap-3">
                <button @click="copyReport" class="w-full py-3 bg-slate-700 text-emerald-400 border border-slate-600 rounded-xl font-bold">複製報表</button>
                <button @click="$emit('settle', rate);showSettlement=false" class="w-full py-3 bg-amber-600 text-white rounded-xl font-bold">結束並存檔</button>
            </div>
        </div></div>
    </div>`,
    setup(props, { emit }) {
        ...
        const copyReport = () => {
            let t = `🎲 局: ${props.game.name}\n📅 ${new Date().toLocaleString()}\n💰 匯率: 1:${rate.value}\n---\n`;
            [...props.game.players].sort((a,b)=>calculateNet(b)-calculateNet(a)).forEach(p=>{
