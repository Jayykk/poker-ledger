import { ref, onMounted, watch, nextTick } from "https://unpkg.com/vue@3/dist/vue.esm-browser.js";
        ...
        <div class="w-full max-w-sm space-y-4">
            ...
            <div class="bg-slate-800 p-5 rounded-2xl border border-slate-700 space-y-3">
                ...
                <button @click="$emit('auth', isReg?'register':'login', form)" :disabled="loading" class="w-full py-3 bg-emerald-600 text-white rounded-xl font-bold">
                    <span v-if="loading && !isGuest" class="loader border-white/30 border-t-white mr-2"></span>
                    {{ isReg ? '註冊' : '登入' }}
                </button>
...
export const LobbyView = {
    ...
    <div class="pt-8 px-4 pb-24">
        ...
        <div class="grid gap-4">
            <div @click="showCreate=true" class="bg-slate-800 p-5 rounded-2xl border border-slate-700 flex items-center gap-4 active:scale-95 transition"><div class="w-12 h-12 rounded-full bg-amber-500/20 text-amber-500 flex items-center justify-center text-xl"><i class="fas fa-plus"></i></div><div><h3 class="text-white font-bold">開新局</h3></div></div>
            <div @click="showJoin=true" class="bg-slate-800 p-5 rounded-2xl border border-slate-700 flex items-center gap-4 active:scale-95 transition"><div class="w-12 h-12 rounded-full bg-emerald-500/20 text-emerald-500 flex items-center justify-center text-xl"><i class="fas fa-sign-in-alt"></i></div><div><h3 class="text-white font-bold">加入房間</h3></div></div>
        </div>
...
export const GameView = {
    props: ['game', 'user'],
    template: `
    <div v-if="!game" class="h-[80vh] flex items-center justify-center text-gray-500">無牌局</div>
    <div v-else class="pt-16 px-4 pb-24">
        ...
        <div class="mt-8 flex gap-3 justify-center">
            <button @click="copyId" class="px-4 py-2 bg-slate-800 text-gray-400 rounded-lg text-xs">複製 ID</button>
...
export const ReportView = {
    ...
    template: `
    <div class="pt-8 px-4 pb-24">
        <h2 class="text-2xl font-bold text-white mb-6">報表</h2>
        ...
        <div class="space-y-3"><div v-for="(h, i) in history" :key="i" class="bg-slate-800 p-4 rounded-xl border border-slate-700 flex justify-between items-center"><div><div class="text-xs text-gray-400">{{ h.dateStr }}</div><div class="text-white font-bold">Game #{{ i+1 }}</div></div><div class="text-right"><div class="font-mono font-bold" :class="h.profit>=0?'text-emerald-400':'text-rose-400'">{{ h.profit>0?'+':''}}{{ formatNumber(h.profit) }}</div></div></div></div>
    </div>`,
    setup(props) {
        let chart = null;
        const render = () => {
...
export const ProfileView = {
    ...
    <div class="pt-8 px-4 pb-24 text-center">
        ...
        <button @click="$emit('logout')" class="w-full mt-8 py-3 bg-slate-800 text-rose-400 border border-slate-700 rounded-xl font-bold">登出</button>
    </div>`
};
