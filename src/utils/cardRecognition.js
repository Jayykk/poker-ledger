...
Respond with JSON only: {"cards":[{"rank":"A","suit":"spades","confidence":0.95}]}`;
...
/**
 * Convert the model's JSON into the app's card strings ("A♠", "10♥").
 * Invalid entries are dropped, duplicates removed, order preserved.
 * @param {unknown} raw `{ cards: [...] }` or a bare array
 * @returns {string[]}
 */
export function normalizeRecognizedCards(raw) {
  const list = Array.isArray(raw)
    ? raw
    : (raw && typeof raw === 'object' && Array.isArray(raw.cards)) ? raw.cards : [];
